...
NSPDK_FeatureGenerator::NSPDK_FeatureGenerator(const std::string& id): FeatureGenerator(id),FlagsServiceClient(id) {
    ...
    new_flag(&mThicknessOpt, "thickness_opt","(bool)\nUse shortest paths union features");    
...
void NSPDK_FeatureGenerator::GenerateFeatures(const GraphClass& aG, const vector<unsigned>& aFirstEndpointList){
  ...
  if (mThicknessOpt){
    //generate features for union of thick shortest paths subgraphs. Note: thickness=-1 means no features 
    for (unsigned t=0;t<=mThickness;t++){
      for (unsigned dt=1;dt<=mThicknessDistance; dt++){//NOTE: distance=0 yields the same features as for the pairs of neighbourhood subgraphs and is thus not recomputed
	GeneratePathFeatures(aG,dt,t,first_endpoint_list);
      }//for dt
    }//for t
  }//if thickness
}
...
void NSPDK_FeatureGenerator::GeneratePathFeatures(const GraphClass& aG, int aDistance, int aThickness, const vector<unsigned>& aFirstEndpointList){
  ThirdOrderHistogramClass feature_list;
  bool empty_flag=true;
  for (unsigned i=0;i<aFirstEndpointList.size();i++){
    unsigned src_id=aFirstEndpointList[i];
	...
	unsigned dest_id=dest_id_list[dest_j];
	if (aG.GetVertexKernelPoint(dest_id) && aG.GetVertexAlive(dest_id)){//proceed to extract features only if the *dest* vertex is a kernel point and is alive
	  //extract hard and soft encoding for path src to dest
	  pair<unsigned,SecondOrderHistogramClass > path=PathGraphCanonicalForm(src_id,dest_id,aG,aThickness,aDistance);
	  feature_list.Add(path.first,path.second);
	  empty_flag=false;
	  if (mDebugVerbosity>0){
	    mDebugInfo.mHashToPlainSubgraphPairMap[path.first]=make_pair(path.first,path.first);
	  }
	}//if dest is of active type
      }//for dest_id
    }//if src is active
  ...
  if (empty_flag){}
    ...
    string distance_thickness_id_str="dt:"+stream_cast<string>(aDistance)+" t:"+stream_cast<string>(aThickness);
    unsigned distance_thickness_id_hash=HashFunc(distance_thickness_id_str);
    mFeatureList[distance_thickness_id_hash]=feature_list;
    if (mDebugVerbosity>0) mDebugInfo.mHashToRadiusDistanceMap[distance_thickness_id_hash]=distance_thickness_id_str;
  }
}

pair<unsigned,SecondOrderHistogramClass > NSPDK_FeatureGenerator::PathGraphCanonicalForm(int aSrcVertexIndex, int aDestVertexIndex, const GraphClass& aG, int aThickness, int aDistance){
  unsigned discrete_encoding=0;
  SecondOrderHistogramClass soft_encoding;
  if (mMatchType == "hard" ){
    discrete_encoding=PathHardEncoding(aSrcVertexIndex,aDestVertexIndex,aG,aThickness,aDistance);
    soft_encoding.Insert(1,1);
    
  } else if (mMatchType == "soft" ){
    discrete_encoding=HardEncoding(aSrcVertexIndex,aG,0)+HardEncoding(aDestVertexIndex,aG,0);
    soft_encoding=PathSoftEncoding(aSrcVertexIndex,aDestVertexIndex,aG,aThickness,aDistance);
...
SecondOrderHistogramClass NSPDK_FeatureGenerator::PathSoftEncoding(int aSrcVertexIndex, int aDestVertexIndex, const GraphClass& aG, int aThickness, int aDistance){
  SecondOrderHistogramClass soft_attribute_list;

  //induce the subgraph from set of vertices in the thick path
  set<unsigned> vset=aG.GetUnionThickShortestPathsVertexIDSet(aSrcVertexIndex, aDestVertexIndex, aDistance, aThickness);

  for (set<unsigned>::iterator it=vset.begin(); it!=vset.end();++it){
...
unsigned NSPDK_FeatureGenerator::PathHardEncoding(int aSrcVertexIndex, int aDestVertexIndex, const GraphClass& aG, int aThickness, int aDistance){
  //induce the subgraph from set of vertices in the thick path
  set<unsigned> vset=aG.GetUnionThickShortestPathsVertexIDSet(aSrcVertexIndex, aDestVertexIndex, aDistance, aThickness);

  GraphClass gal;
  unsigned root=aG.GetVertexInducedRootedSubGraph(vset,aSrcVertexIndex,gal);
  gal.ComputePairwiseDistanceInformation(aDistance+aThickness*2);
  ...
  if (mDebugVerbosity>0){
    ...
    string attribute="graph_id: "+aG.GetGraphID()+
      " vertex_id: s:"+stream_cast<string>(aSrcVertexIndex)+ " d:"+stream_cast<string>(aDestVertexIndex)+
      " thickness: "+stream_cast<string>(aThickness)+" distance: "+ stream_cast<string>(aDistance)+
...
void MNSPDK_FeatureGenerator::GenerateFeatures(const GraphClass& aG, const vector<unsigned>& aFirstEndpointList){
  ...
  if (mThicknessOpt){
    //generate features for union of thick shortest paths subgraphs. Note: thickness=-1 means no features 
    for (unsigned t=0;t<=mThickness;t++){
      for (unsigned d=1;d<=mThicknessDistance; d++){//NOTE: distance=0 yields the same features as for the pairs of neighbourhood subgraphs and is thus not recomputed
	GeneratePathFeatures(aG,d,t,first_endpoint_list);
      }//for dt
    }//for t
  }//if thickness
}
...
void MNSPDK_FeatureGenerator::GeneratePathFeatures(const GraphClass& aG, int aDistance, int aThickness, const vector<unsigned>& aFirstEndpointList){
  string distance_thickness_id_str="dt:"+stream_cast<string>(aDistance)+" t:"+stream_cast<string>(aThickness);
  unsigned distance_thickness_id_hash=HashFunc(distance_thickness_id_str);
  ...
  for (unsigned i=0;i<aFirstEndpointList.size();i++){
    unsigned src_id=aFirstEndpointList[i];
    if (aG.GetVertexKernelPoint(src_id) && aG.GetVertexAlive(src_id)){//proceed to extract features only if the *src* vertex is a kernel point and is alive
      string src_id_str="src:"+stream_cast<string>(src_id);
      vector<unsigned> dest_id_list=aG.GetFixedDistanceVertexIDList(src_id,aDistance);
      for (unsigned dest_j=0;dest_j<dest_id_list.size();dest_j++){
	unsigned dest_id=dest_id_list[dest_j];
	if (aG.GetVertexKernelPoint(dest_id) && aG.GetVertexAlive(dest_id)){//proceed to extract features only if the *dest* vertex is a kernel point and is alive
	  //manage memoization
	  pair<unsigned,SecondOrderHistogramClass > path;
	  string src_dest_id_str=src_id_str+" dest:"+stream_cast<string>(dest_id);
	  unsigned src_dest_id_hash=HashFunc(src_dest_id_str);
	  if (mMemoizedSubgraphEncodingCache.count(graph_id)==0 || 
	      mMemoizedSubgraphEncodingCache[graph_id].count(distance_thickness_id_hash)==0 ||
	      mMemoizedSubgraphEncodingCache[graph_id][distance_thickness_id_hash].count(src_dest_id_hash)==0){
	    //extract hard and soft encoding for path src to dest
	    path=PathGraphCanonicalForm(src_id,dest_id,aG,aThickness,aDistance);
	    mMemoizedSubgraphEncodingCache[graph_id][distance_thickness_id_hash][src_dest_id_hash]=path;
	  }
	  else {
	    path=mMemoizedSubgraphEncodingCache[graph_id][distance_thickness_id_hash][src_dest_id_hash];
	  }
	  feature_list.Add(path.first,path.second);
	  empty_flag=false;
	  if (mDebugVerbosity>0){
