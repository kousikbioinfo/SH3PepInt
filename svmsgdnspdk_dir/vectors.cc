...
void SVector::convert2stl(std::vector<std::pair<unsigned, double> >& vec){
  ...
  for (int i=0; i<npairs; i++)
    {
      std::pair<unsigned,double> p=std::make_pair(pairs[i].i,pairs[i].v);
      vec.push_back(p);
    }
}
...
std::pair<unsigned, double> SVector::extract_component(unsigned aFeatureOrderIndex){
  ...
  const SVector::Pair *pairs = r->pairs;
  int npairs = r->npairs;
  if (npairs<(int)aFeatureOrderIndex) throw std::range_error("Cannot access the specified feature");
